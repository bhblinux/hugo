...
**** 自定义异常
     #+BEGIN_SRC 
     <?php
function my_error($error,$errmes){
    echo $error.$errmes;
}
set_error_handler("my_error",E_WARNING);
//第二个参数最好是E_USER_ERROR E_USER_WARNING E_USEER_NOTICE
$fp=fopen("aaa.txt","r");
     #+END_SRC
**** 错误触发器
     调用触发器
     trigger_error("error message",E_USER_NOTICE);//参数二,设置错误级别,默认E_USER_NOTICE
**** 日志保存
     error_log
**** 异常处理
     #+BEGIN_SRC 
...
** 方法
   1. mysql_fetch_row
   2. mysql_fetch_assoc
   3. mysql_fetch_array //不要用,会返回索引数组和关联数组
...
** cookie
   setCookie 添加
   setCookie("key","val",time()-200); //设置过期时间删除cookie
** session
   session_start()
** 文件
   1. file_get_contents
